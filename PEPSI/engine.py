...
def train_one_epoch_feature(epoch, args, model, processors, criterion, data_loader_dict, 
    ...
    probs = probs if args.probs else [1/len(data_loader_dict)] * len(data_loader_dict)

    for itr, (curr_dataset, (subjects, samples)) in enumerate(metric_logger.log_every(data_loader_dict, max_len, probs, epoch, header=header, modality=args.modality_input, train_limit=args.train_itr_limit)): 
        ...
        for processor in processors:
            outputs = processor(outputs, subjects, curr_dataset)
    ...
    for itr, (curr_dataset, (subjects, samples)) in enumerate(metric_logger.log_every(data_loader_dict, max_len, probs, epoch, header=header, modality=args.modality_input, train_limit=args.train_itr_limit)): 
        ...
        if feat_optimizer is not None:
            for i, param_group in enumerate(feat_optimizer.param_groups):
                param_group["lr"] = feat_lr_scheduler[curr_itr]
                param_group["weight_decay"] = feat_wd_scheduler[curr_itr]
