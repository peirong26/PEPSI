...
from PEPSI.datasets.id_synth_eval import IDSynthEval
...
class ContrastSpecificDataset(IDSynthEval): 
    ...
    def __init__(self, args, data_dir, gt_dir, aux_gt_dir = None, device='cpu'): 
        ...
        try:
            if args.train_txt is not None:
                train_file = open(args.train_txt, 'r')
                subjs = train_file.readlines()
                all = []
                for subj in subjs:
                    all.append(os.path.join(data_dir, subj.strip()))
                self.names = all
        except:
            pass
        ...
        self.gt_postfix = '.' + '.'.join(gt_names[0].split('.')[1:])
        ...
        elif 'pathol' in self.task:
            self.target_name = 'pathol'
        else:
    ...
    def read_target(self, img, loc_list, flip):
        ...
        if 'seg' in self.task or 'bf' in self.task:
            ...
            #utils.viewVolume(GT, names = [self.target_name + '_orig'], save_dir = '')
            #utils.viewVolume(GTdef, names = [self.target_name], save_dir = '')
            if 'anat' in self.task: # subject-robust
                return {self.target_name: GTdef[None]}, {}
            else: # sr, pathol
                return {}, {self.target_name: GTdef[None]}

    def read_aux_target(self, img, loc_list, flip):
        xx2, yy2, zz2, x1, y1, z1, x2, y2, z2 = loc_list
        if x1 is None:
            aux_GT = torch.squeeze(torch.tensor(img.get_fdata().astype(float), dtype=torch.float, device=self.device))
        else:
            aux_GT = torch.squeeze(torch.tensor(img.get_fdata()[x1:x2, y1:y2, z1:z2].astype(float), dtype=torch.float, device=self.device))
        aux_GT = torch.nan_to_num(aux_GT)
        aux_GT[aux_GT < 0.] = 0.
        aux_GTdef = fast_3D_interp_torch(aux_GT, xx2, yy2, zz2, 'linear')
        maxi = torch.max(aux_GTdef)
        aux_GTdef = aux_GTdef / maxi
        if flip:
            aux_GTdef = torch.flip(aux_GTdef, [0]) 
        #utils.viewVolume(GT, names = [self.target_name + '_orig'], save_dir = '')
        #utils.viewVolume(GTdef, names = [self.target_name], save_dir = '')
        return {'aux_image': aux_GTdef[None]}, {}
    ...
    def __getitem__(self, idx):
        ...
        # Read in GT
        if os.path.isfile(os.path.join(self.gt_dir, os.path.basename(self.names[idx]))):
            GT = nib.load(os.path.join(self.gt_dir, os.path.basename(self.names[idx])))
        else:
            GT = nib.load(os.path.join(self.gt_dir, os.path.basename(self.names[idx]).split('.nii')[0] + self.gt_postfix))
        ...
        if self.aux_gt_dir is not None:
            if os.path.isfile(os.path.join(self.aux_gt_dir, os.path.basename(self.names[idx]))):
                aux_GT = nib.load(os.path.join(self.aux_gt_dir, os.path.basename(self.names[idx])))
            else:
                aux_GT = nib.load(os.path.join(self.aux_gt_dir, os.path.basename(self.names[idx]).split('.nii')[0] + self.gt_postfix))
            aux_GT_subject, aux_GT_sample = self.read_aux_target(aux_GT, [xx2, yy2, zz2, x1, y1, z1, x2, y2, z2], flip)

            sample.update(aux_GT_sample) 
            subjects.update(aux_GT_subject)
