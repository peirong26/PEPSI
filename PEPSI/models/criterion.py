...
from PEPSI.models.losses import GradientLoss, gaussian_loss, laplace_loss, l1_loss
...
class SetCriterion(nn.Module):
    """ 
    This class computes the loss for PEPSI.
    """
    def __init__(self, args, weight_dict, loss_names, device):
        ...
        self.loss_regression = uncertainty_loss[args.losses.uncertainty] if args.losses.uncertainty is not None else l1_loss
        self.pathol_attn = args.weights.image_pathol
        ...
        self.loss_map = {
            ...
            'image': self.loss_image_attn if self.pathol_attn > 0. else self.loss_image,
            'image_grad': self.loss_image_grad,
            #'image_grad': self.loss_image_grad_attn if self.pathol_attn > 0. else self.loss_image_grad,  
            'aux_image': self.loss_aux_image_attn if self.pathol_attn > 0. else self.loss_aux_image,
            'aux_image_grad': self.loss_aux_image_grad,
    ...
    def loss_pathol_ce(self, outputs, targets, samples):
        ...
        Cross entropy of pathology segmentation
        """
        loss_pathol_ce = torch.mean(-torch.sum(torch.log(torch.clamp(outputs['pathol'], min=1e-5)) * samples['pathol'], dim=1))
    ...
    def loss_pathol_dice(self, outputs, targets, samples):
        ...
        Dice of pathology segmentation
        """
        loss_pathol_dice = torch.sum((1.0 - 2.0 * ((outputs['pathol'] * samples['pathol']).sum(dim=[2, 3, 4])) 
                                                       / torch.clamp((outputs['pathol'] + samples['pathol']).sum(dim=[2, 3, 4]), min=1e-5)))
    ...
    def loss_aux_image(self, outputs, targets, *kwargs):
        if outputs['aux_image'].shape == targets['aux_image'].shape:
            if self.loss_regression_type != 'l1':
                loss_aux_image = self.loss_regression(outputs['aux_image'], outputs['aux_image_sigma'], targets['aux_image'])
            else:
                loss_aux_image = self.loss_regression(outputs['aux_image'], targets['aux_image'])
        else: 
            loss_aux_image = 0.
        return {'loss_aux_image': loss_aux_image}
    
    def loss_aux_image_grad(self, outputs, targets, *kwargs):
        if outputs['aux_image'].shape == targets['aux_image'].shape:
            loss_aux_image_grad = self.grad(outputs['aux_image'], targets['aux_image'])
        else: 
            loss_aux_image_grad = 0.
        return {'loss_aux_image_grad': loss_aux_image_grad}

    def loss_aux_image_attn(self, outputs, targets, samples):
        if outputs['aux_image'].shape == targets['aux_image'].shape:
            weights = 1. + self.pathol_attn * samples['pathol']
            if self.loss_regression_type != 'l1':
                loss_aux_image = self.loss_regression(outputs['aux_image'], outputs['aux_image_sigma'], targets['aux_image'], weights)
            else:
                loss_aux_image = self.loss_regression(outputs['aux_image'], targets['aux_image'], weights)
            return {'loss_aux_image': loss_aux_image}
        else: 
            loss_aux_image = 0.
        return {'loss_aux_image': loss_aux_image}
    ...
    def loss_image(self, outputs, targets, *kwargs):
        if outputs['image'].shape == targets['image'].shape:
            if self.loss_regression_type != 'l1':
    ...
    def loss_image_attn(self, outputs, targets, samples):
        if outputs['image'].shape == targets['image'].shape:
            weights = 1. + self.pathol_attn * samples['pathol']
            if self.loss_regression_type != 'l1':
                loss_image = self.loss_regression(outputs['image'], outputs['image_sigma'], targets['image'], weights)
            else:
                loss_image = self.loss_regression(outputs['image'], targets['image'], weights)
        else: 
            loss_image = 0.
        return {'loss_image': loss_image}
    
    def loss_image_grad_attn(self, outputs, targets,samples):
        weights = 1. + self.pathol_attn * samples['pathol']
        loss_image_grad = self.grad(outputs['image'], targets['image'], weights)
        return {'loss_image_grad': loss_image_grad}
...
class SetMultiCriterion(SetCriterion):
    """ 
    This class computes the loss for PEPSI with a list of results as inputs.
    """
    def __init__(self, args, weight_dict, loss_names, device):
        """ Create the criterion.
        Parameters:
