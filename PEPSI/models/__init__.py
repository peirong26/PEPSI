...
def process_args(args, task = 'feat'):
    ...
    if not 'contrastive' in task:
        ...
        if 'pathol' in task:
            args.out_channels['pathol'] = 1 
            args.output_names += ['pathol']
            args.target_names += ['pathol']
        if args.losses.implicit_pathol:
            args.output_names += ['implicit_pathol_orig']
            args.output_names += ['implicit_pathol_pred']
            if args.aux_modality is not None:
                args.output_names += ['implicit_aux_pathol_orig']
                args.output_names += ['implicit_aux_pathol_pred']
        if args.aux_modality is not None or 'aux' in task:
            args.out_channels['aux_image'] = 2 if args.losses.uncertainty is not None else 1
            args.output_names += ['aux_image']
            args.target_names += ['aux_image']
            if args.losses.uncertainty is not None:
                args.aux_output_names += ['aux_image_sigma']
...
def get_criterion(args, task, device):
    ...
    if 'pathol' in task:
        ...
        weight_dict.update( {
            'loss_pathol_ce': args.weights.pathol_ce,
            'loss_pathol_dice': args.weights.pathol_dice,
        } )
    if args.losses.implicit_pathol:
        loss_names += ['implicit_pathol_ce', 'implicit_pathol_dice']
        weight_dict.update( {
            'loss_implicit_pathol_ce': args.weights.implicit_pathol_ce,
            'loss_implicit_pathol_dice': args.weights.implicit_pathol_dice,
        } )
        if args.aux_modality is not None:
            loss_names += ['implicit_aux_pathol_ce', 'implicit_aux_pathol_dice']
            weight_dict.update( {
                'loss_implicit_aux_pathol_ce': args.weights.implicit_pathol_ce,
                'loss_implicit_aux_pathol_dice': args.weights.implicit_pathol_dice,
            } )
    if args.aux_modality is not None or 'aux' in task:
        loss_names += ['aux_image', 'aux_image_grad']
        weight_dict.update( {
            'loss_aux_image': args.weights.image,
            'loss_aux_image_grad': args.weights.image_grad,
        } )
    if 'bf' in task:
