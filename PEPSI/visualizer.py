    ...
    def visualize_all(self, subjects, samples, outputs, out_dir, output_names = ['image'], target_names = ['image']):
        ...
        for i, name in enumerate(names): 
            ...
            if len(out_images) > 0:
                ...
                if 'pathol' in samples[0]:
                    curr_target['pathol'] = [self.prepare_for_itk(samples[i_sample]['pathol'][i].data.cpu().numpy().transpose((3, 2, 1, 0))) for i_sample in range(n_samples)]
                ...
                for i_sample, curr_input in enumerate(curr_inputs):
                    ...
                    for target_name in target_names: 
                        if target_name in curr_target: 
                            if not self.subject_robust or 'bias_field' in target_name or 'pathol' in target_name:
                    ...
                    if 'pathol' in curr_target.keys() and 'pathol' not in target_names: 
                        target_list.append(curr_target['pathol'][i_sample]) 
