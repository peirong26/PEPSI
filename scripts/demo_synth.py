...
from PEPSI.visualizer import TaskVisualizer
from PEPSI.datasets import build_dataset_single 
...
def generate(args):
    ...
    if args.device_generator:
        device = args.device_generator
    ...
    dataset.mild_samples = args.mild_samples
    dataset.all_samples = args.all_samples 
    for itr in range(min(args.test_itr_limit, len(dataset.names))):
        
        subj_name = os.path.basename(dataset.names[itr]).split('.nii')[0]

        #if subj_name != 'sub-strokecase0127':
        #    print('SKIP image (%d/%d): %s' % (itr, len(dataset), dataset.names[itr]))
        #    continue
        if subj_name != 'sub-255':
            print('SKIP image (%d/%d): %s' % (itr, len(dataset), dataset.names[itr]))
            continue
        ...
        for i_deform in range(args.num_deformations):
            ...
            if 'aux_image' in subjects:
                utils.viewVolume(subjects['aux_image'], aff, names = ['aux_image_orig'], save_dir = def_save_dir)
            if 'pathol_prob' in subjects:
                utils.viewVolume(subjects['pathol_prob'], aff, names = ['pathol_prob'], save_dir = def_save_dir)
            if 'seg' in args.task:
